...
function startListeningToSocket() {
    function handleWebSocketMessage(data) {
        ...
        const messageHandlers = {
            'rfid_scanned': async () => {
                console.log(Number(data.id) === Number(gameRoomId))
...
function showMessage(msg, untilTime = null) {
    ...
    if (untilTime) {
        ...
        messageUpdateInterval = setInterval(() => {
            ...
            messageArea.innerHTML = `<div class="message display-6">Please hold on for a while, it will be your turn in ${minutesLeft} minute${minutesLeft !== 1 ? 's' : ''}</div>`;
        }, 60000);
    } else {
        messageTimeout = setTimeout(() => {
