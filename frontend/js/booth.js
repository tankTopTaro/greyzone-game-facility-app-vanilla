...
async function handleConfirm() {
    ...
    try {
      if (players.length > 1) {
        ...
        if (res.ok) {
          ...
          showMessage('Follow the pink lights');
        }
      } else {
        wsService.send({ type: 'confirm', from: CLIENT });
        showMessage('Follow the pink lights');
  ...
  function showMessage(msg) {
    confirmArea.innerHTML = `<div class="message">${msg}</div>`;
  ...
  function renderPlayers() {
    ...
    if (players.length === 0) {
      container.innerHTML = `<h1>Please scan your tags</h1>`;
      return;
    }
    ...
    players.forEach(player => {
      ...
      card.innerHTML = `
        <img src="/api/images/players/${player.id}.jpg" alt="${player.nick_name}'s image" />
